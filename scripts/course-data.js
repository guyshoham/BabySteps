...
//
// Both `seed.js` (writes these to Firestore) and `upload-videos.js` (uploads the
// matching video files to R2) read from here, so a lesson's id, order, and r2Key
// can never drift between the database and the bucket.
//
...
// 2. Make every `r2Key` match the object key uploaded to the R2 bucket, exactly
//    (case-sensitive, including the folder prefix). `upload-videos.js` enforces this.
